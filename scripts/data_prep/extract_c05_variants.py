...
Modes:
  ...
  --variant vh_pid60 [--pident-threshold 0.60]
      ...
      pident >= threshold and write c05_vh_pid<PCT>.fasta from the tseq
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--variant", choices=["vh_pid60", "cdrh3_posid"],
    ...
    p.add_argument("--pident-threshold", type=float, default=0.60,
                   help="Full-chain pident cutoff for vh_pid60 variant.")
    p.add_argument("--cdrh3-threshold", type=float, default=None,
                   help="CDR-H3 positional-identity cutoff for cdrh3_posid variant (required for that mode).")
...
def write_vh_pid_variant(
    ...
    print(f"[vh_pid] {len(df):,} unique hits -> {len(keep):,} at pident >= {pident_threshold:.2f}", flush=True)
    output_path.parent.mkdir(parents=True, exist_ok=True)
    with open(output_path, "w") as f:
        for _, row in keep.iterrows():
            f.write(f">{row['target']}\n{row['tseq']}\n")
    print(f"[vh_pid] Wrote {len(keep):,} sequences to {output_path}")
    return len(keep)
...
def main() -> None:
    ...
    if args.variant == "vh_pid60":
        ...
        out = out_dir / f"c05_vh_pid{pct}.fasta"
        write_vh_pid_variant(results_tsv, out, args.pident_threshold)
        return
