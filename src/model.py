"""ESM masked-LM scoring utilities for DPO training and Gibbs generation.

This module provides:
1) model loading (EsmForMaskedLM)
...
try:
	from transformers import EsmForMaskedLM as EsmMaskedLM
except ImportError:  # pragma: no cover
	from transformers import EsmMaskedLM  # type: ignore
...
class ESM2:
	...
	def __init__(self, config: ModelConfig):
		...
		self.model = EsmMaskedLM.from_pretrained(config.esm_model_path).to(self.device)
		self.model.eval()
	...
	def forward_logits(self, tokens: torch.Tensor) -> torch.Tensor:
		...
		if not isinstance(logits, torch.Tensor):
			raise TypeError(f"Expected tensor logits from model(input_ids=tokens), got {type(logits)}")
		return logits
