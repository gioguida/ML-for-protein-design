...
def compute_masked_log_probs_batch(
    ...
    mask_positions are expected to be 0-indexed CDR positions.
    """
    n = len(sequences)
...
def score_double_mutants(
    ...
    logger.info("Scoring %d sequences (4 x %d forward-pass batches)", n, -(-n // batch_size))

    logp_a1 = compute_masked_log_probs_batch(scorer, seqs_a1, pos_a1, tgt_a1, batch_size)
    logp_a2 = compute_masked_log_probs_batch(scorer, seqs_a2, pos_a2, tgt_a2, batch_size)
    logp_b1 = compute_masked_log_probs_batch(scorer, seqs_b1, pos_b1, tgt_b1, batch_size)
    logp_b2 = compute_masked_log_probs_batch(scorer, seqs_b2, pos_b2, tgt_b2, batch_size)
...
def compute_cdr_pseudo_perplexity(
    scorer: ESM2,
    full_sequence: str = WILD_TYPE,
    cdr_start: int = 0,
    cdr_end: Optional[int] = None,
) -> float:
    ...
    """
    if cdr_end is None:
        cdr_end = len(full_sequence)
    cdr_len = cdr_end - cdr_start
    if cdr_len <= 0:
        raise ValueError("cdr_end must be greater than cdr_start.")
    ...
    log_probs = compute_masked_log_probs_batch(
        scorer, sequences, mask_positions, target_aas,
        batch_size=cdr_len,
    )
    ppl = math.exp(-float(np.mean(log_probs)))
    logger.info("CDR pseudo-perplexity: %.2f (mean log P: %.4f)", ppl, np.mean(log_probs))
    return ppl
...
def run_multi_scoring_evaluation(
    ...
    for name, df, enrichment_col in datasets:
        ...
        ds_results = run_scoring_evaluation(
            scorer, df, enrichment_col, wt_sequence, batch_size, seed,
        )
