...
A pipeline config (conf/pipeline/*.yaml) lists stages; each stage names a
task + data group. `run_pipeline` iterates stages, re-materializing the
per-stage config via Hydra compose, and threads the output checkpoint of
...
def compose_stage(
    ...
    """
    base_overrides = [f"task={stage.task}", f"data={stage.data}"]
...
def run_pipeline(cfg: DictConfig) -> None:
    ...
    if cfg.get("pipeline") is None:
        raise ValueError(
            "No pipeline selected. Pass `+pipeline=<name>` on the CLI "
            "(see conf/pipeline/ for available pipelines)."
        )
    ...
    try:
        ...
        original_overrides = []
    cli_overrides = _filter_cli_overrides(original_overrides)
    ...
    if cli_overrides:
        logger.info("CLI overrides re-applied to each stage: %s", cli_overrides)
    ...
    for i, stage in enumerate(stages, start=1):
        ...
        stage_cfg = compose_stage(stage, init_ckpt, cli_overrides)
