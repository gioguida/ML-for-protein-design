...
# unset to keep the legacy single-config behavior. Both presets use 15 chains;
# distribution seeds each chain from a random DMS CDR-H3, fitness from C05 WT.
# ---------------------------------------------------------------------------
case "${GIBBS_CONFIG:-}" in
  distribution)
    N_CHAINS="${N_CHAINS:-15}"
